...
class Nip104 {
  static Future<Event> encodeKeyPackageEvent(
      String encodedKeyPackage, List<List<String>> tags, String myPubkey, String privkey) async {
    return await Event.from(
        kind: 443, tags: tags, content: encodedKeyPackage, pubkey: myPubkey, privkey: privkey);
  }
  ...
  static KeyPackageEvent decodeKeyPackageEvent(Event event) {
    ...
    for (var tag in event.tags) {
      if (tag[0] == 'mls_protocol_version') mlsProtocolVersion = tag[1];
    ...
    return KeyPackageEvent(pubkey, createTime, mlsProtocolVersion ?? '', ciphersuite ?? '',
